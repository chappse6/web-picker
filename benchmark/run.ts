...
export interface BenchmarkFixture {
  ...
  html: string;
  /** Selector used only as benchmark ground truth. */
  target: string;
}
...
export interface BenchmarkResult {
  ...
  textOnlyRate: number;
  locatorRate: number;
  confidenceCalibration: Record<'high' | 'medium' | 'low', CalibrationBucket>;
...
export function runBenchmark(fixtures: readonly BenchmarkFixture[]): BenchmarkResult {
  ...
  const calibration: Record<'high' | 'medium' | 'low', CalibrationBucket> = {
...
function renderMarkdown(result: BenchmarkResult): string {
  ...
  const fixtureRows = result.fixtures
    .map((fixture) => `| ${fixture.id} | ${fixture.category} | ${fixture.textOnlySuccess} | ${fixture.locatorSuccess} | ${fixture.confidence} | ${fixture.payloadBytes} |`)
    .join('\n');
